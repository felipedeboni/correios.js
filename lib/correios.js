module.exports = (function() {
  ...
  var keyOf = function( object, value ){
  ...
  var correios = {
     ...
     * @return {error|data}
     */
    address: function(cep, callback) {
      if ( !callback ) {
        throw new Error('Correios error -> you must specify a callback');
      }

      if ( isZipcodeValid(cep) === false ) {
        return callback( new Error('Correios error -> First parameter must be a valid postal code (cep)') );
      }

      request.get({
        uri: correiosOptions.baseUrls.address.replace('%cep%', cep)
      }, function(error, data) {
        if ( error ) {
          return callback( new Error('Correios error -> ' + error) );
        }
        ...
        try {
          ...
          return callback( new Error('Correios error -> ' + e) );
        }
  ...
  correios.getPrice = correios.price;
